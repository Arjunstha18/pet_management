...
class StudentController {
    ...
    public function store($data)
    {
        ...
        if ($stmt->execute()) {
            $_SESSION['message'] = "Student Created successfully.";
                ...
                header("Location: /~2357488/student_management/public/");
                exit;
        }
    ...
    public function search($query)
    {
        ...
        foreach ($students as $student) {
            ...
            $html .= '<td>';
            $html .= '<a href="/~2357488/student_management/public/edit/' . $student['id'] . '" class="btn btn-warning btn-sm me-2" style="margin-right: 5px; padding: 5px 15px;">Edit</a>';
            $html .= '<a href="#" class="btn btn-danger btn-sm" style="padding: 5px 10px;" 
               onclick="if (confirm(\'Are you sure you want to delete this student?\')) { 
                   fetch(\'/~2357488/student_management/public/delete/' . $student['id'] . '\', { 
                       method: \'POST\' 
                   })
                   .then(response => {
                       if (response.ok) {
